...
def get_age_from(request1):
    if request1.isdigit():
        ...
        age_from = 0
        write_msg(event.user_id, f"Отлично, вам не важен минимальный возраст перейдем к следующему шагу? Максимальный возраст?")
    return age_from
...
def get_age_to(request1):
    if request1.isdigit():
        ...
        age_to = 100
        write_msg(event.user_id, f"Отлично, вам не важен максимальный возраст перейдем к следующему шагу? ГОРОД?")
    return age_to
...
def get_city(request1):
    ...
    write_msg(event.user_id, f"Отлично, вы выбрали город {request1} перейдем к следующему шагу? ")
